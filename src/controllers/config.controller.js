...
const getConfig = async (req, res) => {
    try {
        ...
        if (activeEvent) {
            ...
            if (activeEvent.deliveryMethods && activeEvent.deliveryMethods.length > 0) {
...
const updateConfig = async (req, res) => {
    try {
        ...
        const fields = [
            'taxRate', 'currencySymbol', 'enablePoints', 'enableShipping', 'maintenanceMode',
            'storeName', 'contactEmail', 'contactPhone', 'address',
            'socialInstagram', 'socialFacebook', 'socialTwitter',
            'preventStockout', 'pointsPerCurrency', 'freeShippingThreshold', 'enableEvents',
            'enablePointsRedemption', 'moneyPerPoint', 'logoUrl', 'marqueeText', 
            'bannerImage', 'adImage', 'adText', 'secondaryAds', 'openingHours',
            'enableBranches', 'enableTransfers', 'enableStockMovements', 'enableStockControl',
            'webSafetyStock', 'lowStockThreshold', 'criticalStockThreshold', 'baseCurrency',
            'enableAutoBackup', 'backupFrequency'
        ];
        ...
        if (data.criticalStockThreshold !== undefined) {
            updateData.criticalStockThreshold = parseInt(data.criticalStockThreshold) || 0;
        }

        if (data.customMeasurementUnits) {
            updateData.customMeasurementUnits = Array.isArray(data.customMeasurementUnits) ? data.customMeasurementUnits : [];
        }
        if (data.enabledPaymentMethods !== undefined) {
            updateData.enabledPaymentMethods = Array.isArray(data.enabledPaymentMethods) ? data.enabledPaymentMethods : null;
        }
